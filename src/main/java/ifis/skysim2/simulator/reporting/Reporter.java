package ifis.skysim2.simulator.reporting;
...
public class Reporter {
    ...
    private void setup() {
        widthOfFirstColumn = Math.max(20, Math.max(Math.max(2 * Integer.toString(numberOfTrials).length() + 3, shortName.length()), CALIBRATION.length()));
