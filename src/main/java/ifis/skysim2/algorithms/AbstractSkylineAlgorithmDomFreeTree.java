...
import ifis.skysim2.data.trees.quadtree.AbstractDomFreeQuadTreePL;
...
public abstract class AbstractSkylineAlgorithmDomFreeTree extends AbstractSkylineAlgorithm {
    ...
    private float[] _findUndominatedRandomPresorted(PointSource ds) {
        ...
        for (int posDataStart = pos - 1; posDataStart >= 0; posDataStart -= d) {
            ...
            int posCand = d - 1;
            // potentially equal
            while (candidate[posCand] == data[posData]) {
                posCand--;
                posData--;
                if (candidate[posCand] > data[posData]) {
                    continue dataloop; // equal
                }
            }
            ...
            if (candidate[posCand] >= data[posData]) {
                ...
                // @cand dominates
            } else { // potentially being dominated
                while (posCand > 0) {
                    posCand--;
                    posData--;
                    if (candidate[posCand] > data[posData]) {
